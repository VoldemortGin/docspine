...
| PDF export: `to_pdf()` / `save_pdf()` — flowed layout + pagination; per-section page geometry (`sectPr`), styles.xml + theme effective styles, numbering engine, table fidelity (borders/merges/margins, cross-page; vAlign top-only), paragraph borders/shading, inline images + absolutely-positioned anchored images (no text wrap), `defaultTabStop` tab advance | working |
